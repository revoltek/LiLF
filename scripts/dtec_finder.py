...
def fit_lombscargle(phases, data, tint, ant="CS002LBA", freq_split=0, plot = True):
    ...
    if len(freqs) == 0:
        #print("time interval fully flagged")
    ...
    dtecs, __ = find_peaks(periodogram)
    dtecs = dtecs[np.argsort(periodogram[dtecs])][::-1][:2]
    if periodogram[dtecs[0]]/periodogram[dtecs[1]] > 2:
...
def quality_check(full_dtec, full_dtec_err, full_timesteps, full_ants):
    ...
    if fine > 2:
        print("second half too noisy")
        return 'first'
    elif fine < -2:
        print("first half too noisy")
        return 'second'
    else:
...
def calculate_dtec(solspath, soltab="phase000", antenna="all", solution_interval=10, nstacks=None, split_band=True, constrain_phases=True, plot=False, t_start=None, select_time='all', mode='lombscargle'):
    ...
    for i, ant in enumerate(data['ant']):
        ...
        for t, bin in enumerate(timesplit):
            ...
            if mode == 'lombscargle':
                dtec, dtec_err, freqs, angles = fit_lombscargle(phases, data, tint, ant=ant, freq_split=freq_split, plot=plot)
            elif mode == 'curvefit':
...
def dTEC_fitter(solspath: str) -> None:
    ...
    int_dtec = calculate_dtec(solspath, antenna='international_no_german', solution_interval=2, nstacks=1, select_time=select_time, mode='lombscargle')
