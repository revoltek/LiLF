...
with w.if_todo('cooncat'):
    logger.info('Concatenate data...')
    # concat all SBs
    # SB.MS:CIRC_PHASEDIFF_DATA -> concat.MS:DATA
    lib_util.check_rm('concat_core.MS')
    s.add('DP3 '+parset_dir+'/DP3-concat.parset msin="['+','.join(MSs.getListStr())+']" msout=concat_core.MS', log='concat.log', commandType='DP3')
    s.run(check=True)

### DONE

MSs_concat = lib_ms.AllMSs( ['concat_core.MS'], s, check_flags=False )
...
with w.if_todo('flag'):
    ...
    logger.info("Flagging...")
    MSs.run("DP3 " + parset_dir + "/DP3-flag2.parset msin=$pathMS", log="$nameMS_flag.log", commandType="DP3")
...
with w.if_todo('cal_bp'):
    ...
    MSs.run('DP3 '+parset_dir+'/DP3-cor.parset msin=$pathMS cor.parmdb=cal-pa.h5 \
    ...
    lib_util.check_rm('concat.MS')
    s.add('DP3 msin="['+','.join(MSs.getListStr())+']" msin.datacolumn=SMOOTHED_DATA msout=concat.MS steps=[]', log='concat.log', commandType='DP3')
    s.run(check=True)
    MSs_concat = lib_ms.AllMSs( ['concat.MS'], s, check_flags = False )

    logger.info('Add model of %s from %s to MODEL_DATA...' % (calname, os.path.basename(skymodel)))
    MSs_concat.run("DP3 " + parset_dir + "/DP3-predict.parset msin=$pathMS pre.sourcedb=" + skymodel + " pre.sources=" + calname, \
            log="$nameMS_pre.log", commandType="DP3")

    MSs_concat.run('DP3 ' + parset_dir + '/DP3-soldd.parset msin=$pathMS msin.datacolumn=DATA sol.h5parm=amp-concat.h5 sol.mode=diagonal \
            sol.solint='+str(base_solint)+' sol.nchan=1 sol.smoothnessconstraint=1e6', \
            log='concat_solAMP.log', commandType="DP3")

    lib_util.run_losoto(s, 'amp', ['amp-concat.h5'],
