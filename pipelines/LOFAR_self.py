...
# set image size
imgsizepix = int(2.1*MSs[0].getFWHM(freq='mid')*3600/pixscale) if MSs.isLBA else int(1.9*MSs[0].getFWHM(freq='mid')*3600/pixscale)
...
for c in range(2):
    ...
    with w.if_todo('solve_tec1_c%02i' % c):
        ...
        if MSs.isLBA:
            MSs.run('DPPP '+parset_dir+'/DPPP-solTEC.parset msin=$pathMS sol.h5parm=$pathMS/tec1.h5 \
                    ...
                    sol.uvlambdamin={} sol.solint=15 sol.nchan=8'.format(100*freqscale),
    ...
    with w.if_todo('solve_tec2_c%02i' % c):
        ...
        if MSs.isLBA:
            MSs.run('DPPP '+parset_dir+'/DPPP-solTEC.parset msin=$pathMS sol.h5parm=$pathMS/tec2.h5 \
                    ...
                    sol.solint=1 sol.nchan=4 sol.uvlambdamin={} '.format(100*freqscale), log='$nameMS_solTEC-c'+str(c)+'.log', commandType='DPPP')
            ...
            MSs.run('DPPP ' + parset_dir + '/DPPP-solTEC.parset msin=$pathMS sol.h5parm=$pathMS/tec2.h5 \
                    sol.antennaconstraint=[[CS002HBA0,CS002HBA1,'
                                           ...
                                           'RS306HBA,'
                                           'RS503HBA]] \
                    sol.solint=1 sol.nchan=4 sol.uvlambdamin={}'.format(100*freqscale),
                    log='$nameMS_solTEC-c' + str(c) + '.log', commandType='DPPP')
    ...
    if c == 0:
            ...
            if MSs.isLBA:
                MSs.run('DPPP '+parset_dir+'/DPPP-solG.parset msin=$pathMS sol.h5parm=$pathMS/g1.h5' \
                    'sol.antennaconstraint = [[CS001LBA, CS002LBA, CS003LBA, CS004LBA, CS005LBA, CS006LBA, CS007LBA,'
                                              ...
                                              'CS302LBA, CS401LBA, CS501LBA]] sol.uvlambdamin={}'.format(
                    100*freqscale), log='$nameMS_solG1-c'+str(c)+'.log', commandType='DPPP')
            elif MSs.isHBA:
                MSs.run('DPPP ' + parset_dir + '/DPPP-solG.parset msin=$pathMS sol.h5parm=$pathMS/g1.h5 \
                        sol.antennaconstraint=[[CS002HBA0,CS002HBA1,'
                                               'CS003HBA0,CS003HBA1,'
                                               ...
                                               'CS302HBA0,CS302HBA1,'
                                               'CS401HBA0,CS401HBA1,'
                                               'CS501HBA0,CS501HBA1]] sol.uvlambdamin={}'.format(
                        100*freqscale),
        ...
        if solve_gain:
            with w.if_todo('solve_g_c%02i' % c):
                ...
                MSs.run('DPPP '+parset_dir+'/DPPP-solG.parset msin=$pathMS sol.h5parm=$pathMS/g2.h5 sol.uvlambdamin={}'.format(100*freqscale), \
    ...
    with w.if_todo('imaging_c%02i' % c):
        ...
        if c == 0:
            kwargs = {'do_predict': True, 'baseline_averaging': '', 'auto_mask':2.5}
        else:
            kwargs = {'fits_mask': maskname, 'use_idg': '', 'grid_with_beam': '', 'use_differential_lofar_beam': '',
                      'auto_mask': 2.0}
        if MSs.isLBA:
            kwargs.update({'minuv_l': 30, 'maxuv_l': 4500, 'parallel_deconvolution': 512,
                           'channels_out': MSs.getChout(4.e6)})
        elif MSs.isHBA:
            kwargs.update({'minuv_l': 80, 'maxuv_l': 12000, 'channels_out': MSs.getChout(8.e6)})
        ...
        lib_util.run_wsclean(s, 'wsclean-c' + str(c) + '.log', MSs.getStrWsclean(), name=imagename, save_source_list='',
                         ...
                         local_rms='', auto_threshold=1.5, multiscale='', multiscale_scales='0,10,20,40', \
