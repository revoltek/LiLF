...
for cmaj in range(maxIter):
    ...
    for dnum, d in enumerate(directions):
        ...
        if w.todo('c%02i-%s-predict' % (cmaj,d.name)):
            ...
            MSs_dir.run('DPPP '+parset_dir+'/DPPP-predict.parset msin=$pathMS msin.datacolumn=DATA msout.datacolumn=MODEL_DATA \
                    ...
                    log='$nameMS_pre-c'+str(cmaj)+'-'+d.name+'.log', commandType='DPPP')
        ...
        if w.todo('c%02i-%s-preimage' % (cmaj,d.name)):
            ...
            clean('%s-pre' % (d.name), MSs_dir, res='normal', size=d.size)
        ...
        d.skymodel = 'img/ddcalM-%s-pre-sources.txt' % (d.name)
        d.skydb = 'img/ddcalM-%s-pre-sources.skydb' % (d.name)
        ...
        for cdd in range(10):
            ...
            ##########################################################
            ## Predict - ms:MODEL_DATA
            #if w.todo('c%02i-%s-cdd%02i-predict' % (cmaj,d.name,cdd)):
            ...
            #        log='$nameMS_pre-c'+str(cmaj)+'-'+d.name+'-cdd'+str(cdd)+'.log', commandType='DPPP')

            #    w.done('c%02i-%s-cdd%02i-predict' % (cmaj,d.name,cdd))
            #### DONE
            ...
            if w.todo('c%02i-%s-cdd%02i-calibrate' % (cmaj,d.name,cdd)):
                ...
                lib_util.run_losoto(s, 'ph', [ms+'/cal-ph.h5' for ms in MSs_dir.getListStr()], \
                    [parset_dir+'/losoto-plot-tec.parset'], plots_dir='ddcal/plots/plots-c%i-%s-cdd%i' % (cmaj, d.name, cdd))
                ...
                MSs_dir.run('DPPP '+parset_dir+'/DPPP-correct.parset msin=$pathMS msin.datacolumn=DATA msout.datacolumn=CORRECTED_DATA \
                             cor.parmdb=ddcal/solutions/cal-ph-c'+str(cmaj)+'-'+d.name+'-cdd'+str(cdd)+'.h5 cor.correction=tec000', \
                ...
                if doamp:
                    ...
                    MSs_dir.run('DPPP '+parset_dir+'/DPPP-solG.parset msin=$pathMS msin.datacolumn=SMOOTHED_DATA sol.mode=diagonal \
                        ...
                        sol.h5parm=$pathMS/cal-amp1.h5 sol.uvmmin=300 sol.solint='+str(solint_amp), \
                    ...
                    lib_util.run_losoto(s, 'amp1', [ms+'/cal-amp1.h5' for ms in MSs_dir.getListStr()], \
                        [parset_dir+'/losoto-amp.parset', parset_dir+'/losoto-plot2.parset'], plots_dir='ddcal/plots/plots-c%i-%s-cdd%i' % (cmaj, d.name, cdd))
                    ...
                    lib_util.run_losoto(s, 'amp2', [ms+'/cal-amp2.h5' for ms in MSs_dir.getListStr()], \
                        [parset_dir+'/losoto-amp.parset', parset_dir+'/losoto-plot3.parset'], plots_dir='ddcal/plots/plots-c%i-%s-cdd%i' % (cmaj, d.name, cdd))
            ...
            if w.todo('c%02i-%s-cdd%02i-image' % (cmaj,d.name,cdd)):
                ...
                clean('%s-cdd%02i' % (d.name, cdd), MSs_dir, res='normal', size=d.size)
            ...
            # update skymodel
            d.skymodel = 'img/ddcalM-%s-cdd%02i-sources.txt' % (d.name, cdd)
            d.skydb = 'img/ddcalM-%s-cdd%02i-sources.skydb' % (d.name, cdd)
        ...
        # always use the one but last solutions, it's either better or almost equal
        d.h5parms['ph'] = 'ddcal/solutions/cal-ph-c'+str(cmaj)+'-'+d.name+'-cdd'+str(cdd-1)+'.h5'
        d.h5parms['amp1'] = 'ddcal/solutions/cal-amp1-c'+str(cmaj)+'-'+d.name+'-cdd'+str(cdd-1)+'.h5'
        d.h5parms['amp2'] = 'ddcal/solutions/cal-amp2-c'+str(cmaj)+'-'+d.name+'-cdd'+str(cdd-1)+'.h5'
        ...
        if w.todo('c%02i-%s-subtract' % (cmaj,d.name)):
            ...
            # remove CCs that are outsie the d.size region
            ...
            MSs.run('DPPP '+parset_dir+'/DPPP-correct.parset msin=$pathMS msin.datacolumn=MODEL_DATA msout.datacolumn=MODEL_DATA \
                        cor.invert=False cor.parmdb='+d.h5parms['ph']+' cor.correction=tec000', \
                        log='$nameMS_correct-c'+str(cmaj)+'-'+d.name+'.log', commandType='DPPP')
            MSs.run('DPPP '+parset_dir+'/DPPP-correct.parset msin=$pathMS msin.datacolumn=MODEL_DATA msout.datacolumn=MODEL_DATA \
                        cor.invert=False cor.parmdb='+d.h5parms['ph']+' cor.correction=phase000', \
                        log='$nameMS_correct-c'+str(cmaj)+'-'+d.name+'.log', commandType='DPPP')
            if os.path.exists(d.h5parms['amp1']):
                logger.info('Corrupt amp...')
                MSs.run('DPPP '+parset_dir+'/DPPP-correct.parset msin=$pathMS msin.datacolumn=MODEL_DATA msout.datacolumn=MODEL_DATA \
                       cor.invert=False cor.parmdb='+d.h5parms['amp1']+' cor.correction=amplitude000', \
                       log='$nameMS_correct-c'+str(cmaj)+'-'+d.name+'.log', commandType='DPPP') 
                MSs.run('DPPP '+parset_dir+'/DPPP-correct.parset msin=$pathMS msin.datacolumn=MODEL_DATA msout.datacolumn=MODEL_DATA \
                       cor.invert=False cor.parmdb='+d.h5parms['amp2']+' cor.correction=amplitude000', \
                       log='$nameMS_correct-c'+str(cmaj)+'-'+d.name+'.log', commandType='DPPP') 
    ...
    all_h5parms = [d.h5parms['ph'] for d in directions] # phase solutions TODO: add amp
