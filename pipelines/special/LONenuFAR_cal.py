...
less_aggressive_flag = parset.getboolean('LOFAR_cal',
...
with w.if_todo('cleaning'):
    logger.info('Cleaning...')
    lib_util.check_rm(
        'plots-preiono plots-preiono-cs plots-fr plots-bp plots-bp-sub plots-fj plots-iono plots-iono-cs plots-pa plots-amp plots-weights plots-test*}')
...
with w.if_todo('cal_pa'):
    ...
    if MSs_concat_all.hasIS:
        ...
        logger.info(f'Add model of {calname} from {os.path.basename(skymodel)} to MODEL_DATA...')
        MSs_pa.run(f"DP3 {parset_dir}/DP3-predict.parset msin=$pathMS pre.sourcedb={skymodel} pre.sources={calname}", log="$nameMS_pre.log",
                   commandType="DP3")
        if use_spinifex:
        ...
        # predict the element-corrupted model
        raise NotImplementedError('Need to change commute order if no IS.')
        logger.info(f'Add beam-corrupted model of {calname} from {os.path.basename(skymodel)} to MODEL_DATA_COR...')
...
with w.if_todo('cal_fr'):
    ...
    if (min(MSs_concat_all.getFreqs()) < 28.e6):
...
if imaging:
    with w.if_todo('cal_final'):
        ...
        MSs_concat_all.run(f'DP3 {parset_dir}/DP3-cor.parset msin=$pathMS msout.datacolumn=FR_CORRECTED_DATA \
                            cor.parmdb=cal-fr.h5 cor.correction=rotationmeasure000', log='$nameMS_corFR.log',
                           commandType="DP3")
