...
def make_voronoi_reg(directions, fitsfile, outdir_reg='regions', out_mask='facet.fits', png=None):
    ...
    for i, direction in enumerate(directions):
        ...
        if x < 0 or x > data.shape[0] or y < 0 or y > data.shape[1]:
            logger.info(
                'Direction %s is outside the primary beam and will not have a facet (it will still be a calibrator).' % direction.name)
        else:
    ...
    regions.write(regionfile)
    logger.debug('There are %i regions within the PB and %i outside (no facet).' % (
...
def voronoi_finite_polygons_2d_box(vor, box):
    ...
    corners = [(box[0, 0], box[0, 1]), (box[0, 0], box[1, 1]), (box[1, 0], box[0, 1]),
               (box[1, 0], box[1, 1])]  # BL, TL, BR, TR
    dists = []
    for p in vor.points:
    ...
    for p1, region in enumerate(vor.point_region):
        ...
        corners = [(box[0, 0] - radius, box[0, 1] - radius), (box[0, 0] - radius, box[1, 1] + radius),
                   (box[1, 0] + radius, box[0, 1] - radius), (box[1, 0] + radius, box[1, 1] + radius)]
    ...
    for p in poly:
        ...
        coords, idx = np.unique([x + 1j * y for (x, y) in newpolyPath.vertices],
                                return_index=True)  # using complex; a way for grouping
...
class Grouper(object):
    ...
    def __init__(self, coords, fluxes, kernel_size=0.2, look_distance=0.3, grouping_distance=0.03):
        ...
        self.clusters = []
        logger.debug("Grouper: kernel_size=%.1f; look_distance=%.1f; grouping_distance=%.2f" % (
        kernel_size, look_distance, grouping_distance))
    ...
    def run(self):
        ...
        for it in range(self.n_iterations):
            ...
            for i, x in enumerate(self.coords):
                ...
                weights *= self.fluxes[
                               idx_neighbours] ** 2  # multiply by flux**1.5 to make bright sources more important
    ...
    def grouping(self):
        ...
        while len(coords_to_check) > 0:
            idx_cluster = self.neighbourhood_points(coords_to_check[0], self.coords,
                                                    max_distance=self.grouping_distance)
            idx_cluster_to_remove = self.neighbourhood_points(coords_to_check[0], coords_to_check,
                                                              max_distance=self.grouping_distance)
