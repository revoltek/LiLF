...
from LiLF import lib_ms, lib_util, lib_log, lib_cat, lib_img, lib_scheduler, lib_walker
...
def run(step):
    ...
    ddparallel_dir = step['ddparallel_dir']
    ddserial_dir = step['ddserial_dir']
    ...
    if os.path.exists(ddserial_dir):
        ...
        img = lib_img.Image(ddserial_dir+'/c00/images/wideDDS-lres-c0-MFS-residual.fits')
        qdict['ddserial_lres_c0_rms'] = img.getNoise(useMask=False)
        logger.info('ddserial lres residual rms noise (cycle 0): %.1f mJy/b' % (qdict['ddserial_lres_c0_rms']*1e3))
        ...
        with w.if_todo('process_ddimage'):
            ...
            os.chdir('../../../')
            img.write_catalog(outfile='quality/wideDDS-c0-MFS-image-pb.cat.fits', catalog_type='srl', format='fits',
                              correct_proj='True', clobber=True)
