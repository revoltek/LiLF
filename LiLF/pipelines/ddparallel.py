...
########################################################
from LiLF import lib_ms, lib_img, lib_util, lib_log, lib_dd, lib_h5, lib_dd_parallel, lib_cat, lib_scheduler, lib_walker
...
def run(step):
    ...
    subfield_min_flux = step['subfield_min_flux']
    subfield = step['subfield']
    maxIter = step['maxIter']
    maxSubIter = step['maxSubIter']
    phaseSolMode = step['ph_sol_mode']
    remove3c = step['remove3c']
    fulljones = step['fulljones']
    min_facets = step['min_facets']
    max_facets = step['max_facets']
    develop = step['develop']
    use_shm = step['use_shm']
    data_dir = step['data_dir']
    start_sourcedb = step['start_sourcedb']
    userReg = step['userReg']
    sf_phaseSolMode = 'phase'
            ...
            if len(_3c_patches) > 0:
                with w.if_todo('3c_solve_amp'):
                    ...
                    MSs.run(f'DP3 {parset_dir}/DP3-soldd.parset msin=$pathMS msin.datacolumn=SMOOTHED_DATA sol.model_weighted_constraints=true\
                              ...
                              log=f'$nameMS_solamp_3c_c{c}.log', commandType="DP3", max_proc=4)
