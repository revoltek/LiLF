...
with w.if_todo('cal_bp'):
    ...
    logger.info('Concatenating...')
    lib_util.check_rm('concat.MS')
    s.add('DP3 msin="['+','.join(MSs.getListStr())+']" msin.datacolumn=CORRECTED_DATA msout=concat.MS steps=[]', log='concat.log', commandType='DP3')
    s.run(check=True)
    MSs_concat = lib_ms.AllMSs( ['concat.MS'], s, check_flags = False )

    # Smooth data CORRECTED_DATA -> SMOOTHED_DATA (BL-based smoothing)
    # work with the concat so BLsmooth can also be run in frequency space (no -q) and improves the S/N
    logger.info('BL-smooth...')
    MSs_concat.run(f'BLsmooth.py -r -c 8 -n 8 -f {1e-2 if MSs.isLBA else .2e-3} -i DATA -o SMOOTHED_DATA $pathMS', log='$nameMS_smooth3.log',
            commandType ='python')
    ...
    MSs_concat.run('DP3 ' + parset_dir + '/DP3-soldd.parset msin=$pathMS sol.h5parm=$pathMS/amp.h5 sol.mode=diagonal \
