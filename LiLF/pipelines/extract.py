...
from LiLF import lib_ms, lib_img, lib_util, lib_log, lib_scheduler, lib_walker


def run(step):
    coords = step['radec']
    pathdir = step['path']
    ztarget = step.get('redshift', -99)
    targetname = step.get('name', None)
    beam_cut = step['beam_cut']
    no_selfcal = step['no_selfcal']
    extractreg = step.get('extractRegion', None)
    userReg = step.get('userReg', None)
    ampcal = step['ampcal']
    ampSolMode = step['amp_sol_mode']
    phSolMode = step['ph_sol_mode']
    maxniter = step['max_niter']
    subtract_reg_file = step.get('subtract_region', None)
    use_idg = step.get('idg', 'True')
    ...
    def clean(p, MSs, res='normal', size=[1, 1], empty=False, userReg=None, apply_beam=False, datacol=None, minuv=30, numiter=100000, fits_mask=None, update_model=True):
        ...
        if empty:
            ...
            os.system('cat '+log_dir+'/wscleanB-' + str(p) + '.log | grep "background noise"')
    ...
    with w.if_todo('smooth'):
        ...
        MSs_extract.run('BLsmooth.py -c 1 -n 8 -r -i DATA -o SMOOTHED_DATA $pathMS', log='$nameMS_smooth.log', commandType='python', max_proc=1)
